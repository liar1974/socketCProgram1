/*
** Library2.c has two parts. One is used for giving the book titles to the database through
...
#define MYL2PORT "21823"// Library's static port to establish TCP connection
...
int TCPforPhase3()
{
	...
	hints.ai_flags = AI_PASSIVE; // it means use my own IP
        ...
        printf("<Library2> has TCP port 21823 and IP address %s for Phase 3\n",inet_ntoa(*((struct in_addr*)host -> h_addr_list[0])));  
     
	if ((rv = getaddrinfo("nunki.usc.edu", MYL2PORT, &hints, &servinfo)) != 0) {
		   ...
		   if(strcmp(recvBookTitle,bookTitle[compareCounter]) == 0){
                    ...
                    if((numbytes = recv(new_fd, buf, MAXDATASIZE-1, 0)) == -1){
                      ...
                      exit(1);
                            ...
                            printf("<Library 2> received query for <%s> from <User %d>\n",recvBookTitle, numbytes-1);
		            printf("<Library 2> sent the description of <%s> to <User %d>\n", bookTitle[compareCounter], numbytes-1);
		    }
		 }
...
int main(int argc, char *argv[])
{
	...
	//Open the file	
    if((fp1 = fopen("library2.txt", "r")) == NULL){
		printf("Could not open library2.txt\n");
		exit(1);
	}
	...
	if ((numbytes = sendto(sockfd, bookTitle[sentCounter], strlen(bookTitle[sentCounter]), 0,
        ...
        // use getsockname to get the port number
	      ...
	      getsockname(sockfd,(struct sockaddr *)&sin, &len);
              printf("<Library 2> has UDP port %d and IP address %s for Phase 1\n",ntohs(sin.sin_port),inet_ntoa(*((struct in_addr*)host->h_addr_list[0])));
              loopJump = 1;
	  }
        printf("<Library2> has sent <%s> to the database.\n",bookTitle[sentCounter]);
	}
        printf("Updating the database is done for <Library2>\n");
        printf("End of Phase1 for <Library2>.\n");
