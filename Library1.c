/*
** Library1.c has two parts. One is used for giving the book titles to the database through
...
#define MYL1PORT "21723"    // Library's static port to establish TCP connection
...
int TCPforPhase3()
{
	...
	hints.ai_flags = AI_PASSIVE; // it means use my own IP
        ...
        printf("<Library1> has TCP port 21723 and IP address %s for phase3\n",inet_ntoa(*((struct in_addr*)host->h_addr_list[0])));
        
	if ((rv = getaddrinfo("nunki.usc.edu", MYL1PORT, &hints, &servinfo)) != 0) {
		   ...
		   for(compareCounter = 0; compareCounter <= 4; compareCounter++){
				 if(strcmp(recvBookTitle,bookTitle[compareCounter]) == 0){
                    ...
                    if((numbytes = recv(new_fd, buf, MAXDATASIZE-1, 0)) == -1){
                       ...
                       printf("<Library 1> received query for <%s> from <User %d>\n",recvBookTitle, numbytes-1);
		       printf("<Library 1> sent the description of <%s> to <User %d>\n", bookTitle[compareCounter], numbytes-1);
		    }
		 }
...
int main(int argc, char *argv[])
{
	...
	//Open the file
        if((fp1 = fopen("library1.txt", "r")) == NULL){
		printf("Could not open library1.txt\n");
		exit(1);
	}
    ...
    //Send the book title to the database through the UDP
 	for(sentCounter = 0; sentCounter <= 4; sentCounter++){
	  if ((numbytes = sendto(sockfd, bookTitle[sentCounter], strlen(bookTitle[sentCounter]), 0,p->ai_addr, p->ai_addrlen)) == -1) {
		  perror("talker: sendto");
		  exit(1);
	  }
	  if(loopJump == 0){
	    // use getsockname function to get the port number
	  getsockname(sockfd,(struct sockaddr *)&sin, &len);
          printf("<Library 1> has UDP port %d and IP address %s for Phase 1\n",ntohs(sin.sin_port),inet_ntoa(*((struct in_addr*)host->h_addr_list[0])));
          loopJump = 1;
	  }
        printf("<Library1> has sent <%s> to the database.\n",bookTitle[sentCounter]);
	}
        printf("Updating the database is done for <Library1>\n");
        printf("End of Phase1 for <Library1>.\n");
